...
`feat/hero-video-homepage`.~~ **Merged into `main` 2026-09-17** (see that date's entry above) —
`feat/hero-video-homepage` remains unmerged, not part of this adoption. See vault
`SESSION_LOG.md`/`DECISION_LOG.md` for this project (2026-07-24 entries) for the full original
record, including a directory-permissions bug hit and fixed during the first deploy. The two live
review subdomains carry a deploy-only patch to `animations.js` (never committed to either branch)
that disables the reduced-motion/viewport gate so the video always plays for reviewers regardless
of local settings — reapply that patch if either subdomain is ever redeployed from its branch
again; irrelevant to the merged `main` copy, which keeps the real accessibility-gated behavior.
