...
| R-003 | ~~Hosting platform unconfirmed — owner indicated site "may go on Wix"~~ — **materialized, 2026-09-17**: client is now actually hosted on Wix, temporarily, until their current subscription contract ends (end date unknown); this repo is the return-target, not abandoned | High | ~~Medium~~ N/A — no longer a risk, it's the current state | This repo continues to accept improvement work so it's ready whenever the client returns off Wix (ADR-027) | Owner | Closed — see `DECISION_LOG.md` ADR-027 |
