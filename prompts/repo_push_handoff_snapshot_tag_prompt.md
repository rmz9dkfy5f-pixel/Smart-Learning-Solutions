...
## 9. Create a Snapshot Folder Named After the Commit Tag

Every Git commit must have a matching snapshot, matching tag, and real commit hash.

The snapshot folder must be named exactly after the commit tag.
...
Create a folder inside that path named after the commit tag.
...
If the correct snapshot destination cannot be determined, stop before pushing and report the issue clearly.
